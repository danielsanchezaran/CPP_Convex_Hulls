...
struct Point {
 public:
  ...
  void computeAngle() { angle = std::atan2(y, x); }
...
struct Matrix {
 public:
  ...
  double getDeterminant() { return x_00 * x_11 - x_01 * x_10; }
};
 ...
 * the other polygon Compute and find the intersection points between each
 * polygon Compute the Polygon shaped by these vertices by ordering them CCW. A
 * Polygon is tagged as "to be eliminated" if the resulting intersection polygon
 * has an area that is greater than the input overlapping_percent of the Polygon
 * area.
 * @param input: Vector of Convex Hulls.
 * @param overlapping_percent: How much % of the overlaped area of a polygon is
 * necessary to consider it "eliminated"
